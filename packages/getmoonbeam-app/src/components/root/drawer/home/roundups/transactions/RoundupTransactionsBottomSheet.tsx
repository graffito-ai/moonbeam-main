...
import {LoggingLevel, OsType, RoundupTransactionsStatus, TransactionType} from "@moonbeam/moonbeam-models";

/**
 * Interface to be used for determining the location of the roundup transaction
 * store, to be used when displaying it on a map.
 */
interface RoundupTransactionsStoreLocation {
    latitude: number,
    longitude: number,
 ...
 */
export const RoundupTransactionsBottomSheet = (props: {
    brandName: string,
    brandImage: string,
    transactionStoreAddress?: string,
    transactionOnlineAddress?: string,
    transactionAmount: string,
    transactionRoundupAmount: string,
    ...
    const [roundupTransactionStoreGeoLocation, setRoundupTransactionStoreGeoLocation] = useState<RoundupTransactionsStoreLocation | null>(null);
     ...
     */
    useEffect(() => {
        mapViewRef && mapViewRef.current && !roundupTransactionStoreGeoLocation && props.transactionStoreAddress && retrieveStoreGeolocation();
    }, [roundupTransactionStoreGeoLocation, mapViewRef]);

    // retrieve the geolocation (latitude and longitude of the store which the roundup transaction was made at)
    const retrieveStoreGeolocation = async (): Promise<void> => {
        ...
        if (foregroundPermissionStatus.status !== 'granted') {
            ...
            let transactionStoreGeoLocation: RoundupTransactionsStoreLocation | null = null;
            if (geoLocation) {
                setLoadingSpinnerShown(false);
            ...
            setRoundupTransactionStoreGeoLocation(transactionStoreGeoLocation);
            setShowRoundupTransactionBottomSheet(true);
        }
    }

    // return the component for the RoundupTransactionsBottomSheet, part of the RoundupsDashboard page
    return (
        <>
            <Portal>
            ...
            <SafeAreaView
                // @ts-ignore
                style={[StyleSheet.absoluteFill, styles.transactionParentView, {backgroundColor: '#313030'}]}>
                            ...
                            <View style={styles.transactionDetailsView}>
                                ...
                                <View style={styles.brandDetailsView}>
                                    <Text style={styles.transactionDiscountAmount}>
                                        {`$ ${props.transactionRoundupAmount}`}
                        ...
                        <>
                            ...
                            }
                            {
                                ...
                                <View style={styles.transactionMapView}>
                                    ...
                                    >
                                        {
                                            roundupTransactionStoreGeoLocation &&
                                            <Marker
                                                onPress={async () => {
                                                    await retrieveStoreGeolocation();
                                                }}
                                                coordinate={{
                                                    latitude: roundupTransactionStoreGeoLocation.latitude!,
                                                    longitude: roundupTransactionStoreGeoLocation.longitude!
                                                }}
                                            >
                                                <TouchableOpacity
