...
export class ReimbursementsResolverStack extends Stack {
    ...
    constructor(scope: Construct, id: string, props: StackProps & Pick<StageConfiguration, 'environmentVariables' | 'stage' | 'reimbursementsConfig'> & { graphqlApiId: string, graphqlApiName: string }) {
        ...
        reimbursementsLambdaSource.createResolver(`${props.reimbursementsConfig.getReimbursementsResolverName}-${props.stage}-${props.env!.region}`, {
            typeName: "Query",
            fieldName: `${props.reimbursementsConfig.getReimbursementsResolverName}`
        });
        ...
        const reimbursementsTable = new aws_dynamodb.Table(this, `${props.reimbursementsConfig.reimbursementsTableName}-${props.stage}-${props.env!.region}`, {
             ...
             * and the timestamp represents the creation time of a reimbursement (aka when it occurred, in a UNIX format).
        ...
        reimbursementsLambda.addToRolePolicy(
            ...
            new PolicyStatement({
                    ...
                    resources: [
                        ...
                        `${reimbursementsTable.tableArn}/index/${props.reimbursementsConfig.reimbursementsStatusLocalIndex}-${props.stage}-${props.env!.region}`,
                        `${reimbursementsTable.tableArn}/index/${props.reimbursementsConfig.reimbursementsIdGlobalIndex}-${props.stage}-${props.env!.region}`
                    ]
                }
            )
        );
        ...
        reimbursementsLambda.addToRolePolicy(
            new PolicyStatement({
                ...
                resources: [
                    ...
                    ...props.stage === Stages.PROD ? ["arn:aws:secretsmanager:us-wewest-2:251312580862:secret:moonbeam-internal-secret-pair-prod-us-west-2-9xP6tj"] : []
                ]
            })
        );
