import {aws_appsync, aws_dynamodb, StackProps, aws_lambda, aws_lambda_nodejs, Duration, Stack} from "aws-cdk-lib";
...
export class EarningsSummaryResolverStack extends Stack {
    ...
    constructor(scope: Construct, id: string, props: StackProps & Pick<StageConfiguration, 'environmentVariables' | 'stage' | 'earningsSummaryConfig'> & { graphqlApiId: string, graphqlApiName: string }) {
        ...
        // create a new Lambda function to be used as the Producer Lambda for processing and/or creating the earnings daily summaries
        const earningsDailySummaryProducerLambda = new aws_lambda_nodejs.NodejsFunction(this, `${props.earningsSummaryConfig.earningsSummaryProducerFunctionName}-${props.stage}-${props.env!.region}`, {
            functionName: `${props.earningsSummaryConfig.earningsSummaryProducerFunctionName}-${props.stage}-${props.env!.region}`,
            entry: path.resolve(path.join(__dirname, '../../../moonbeam-earnings-summary-producer-lambda/src/lambda/main.ts')),
            handler: 'handler',
            runtime: aws_lambda.Runtime.NODEJS_18_X,
            // we add a timeout here different from the default of 3 seconds, since we expect queries and filtering calls to take longer
            timeout: Duration.seconds(20),
        ...
        // create a CRON expression trigger that will enable running the earnings summaries producer lambda to run on a schedule
        new Rule(this, `${props.earningsSummaryConfig.earningsSummaryCronRuleName}-${props.stage}-${props.env!.region}`, {
            ...
            targets: [new LambdaFunction(earningsDailySummaryProducerLambda, {
                event: events.RuleTargetInput.fromObject({
                    ...
                    detail: {
                        eventType: 'EarningsDailySummaryEvent'
                    }
                })
            })]
        });
        // enable the Lambda function the retrieval of the Moonbeam internal API secrets
        earningsDailySummaryProducerLambda.addToRolePolicy(
            new PolicyStatement({
                effect: Effect.ALLOW,
                actions: [
                    "secretsmanager:GetSecretValue"
        ...
        earningsDailySummaryProducerLambda.addEnvironment(`${Constants.MoonbeamConstants.ENV_NAME}`, props.stage);
    }
}
