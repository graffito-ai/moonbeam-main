...
EXPORTS_FILE=$(dirname "$(pwd)")/moonbeam-cdk/exports/cdk-exports-$env.json
AMPLIFY_APP_ID=$(< "$EXPORTS_FILE" cat | jq '."amplify-dev-us-west-2".amplifyappid')
REGION=$(< "$EXPORTS_FILE" cat | jq '."amplify-dev-us-west-2".awsprojectregion')
DEPLOYMENT_BUCKET=$(< "$EXPORTS_FILE" cat | jq '."amplify-dev-us-west-2".amplifydeploymentbucket')
USER_POOL_ID=$(< "$EXPORTS_FILE" cat | jq '."amplify-dev-us-west-2".awsuserpoolsid')
COGNITO_WEB_CLIENT_ID=$(< "$EXPORTS_FILE" cat | jq '."amplify-dev-us-west-2".awsuserpoolswebclientid')
COGNITO_IDENTITY_POOL_ID=$(< "$EXPORTS_FILE" cat | jq '."amplify-dev-us-west-2".awscognitoidentitypoolid')
