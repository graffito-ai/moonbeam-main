...
export const getStorage = async (fieldName: string, getStorageInput: GetStorageInput): Promise<StorageResponse> => {
    try {
        ...
        switch (getStorageInput.type) {
            ...
            case FileType.Logofiles:
                const logoBucketName = `${Constants.StorageConstants.MOONBEAM_LOGO_FILES_BUCKET_NAME}-${process.env.ENV_NAME!}-${region}`;
                const logoCloudFrontDistributionDomain = process.env.MOONBEAM_LOGO_FILES_CLOUDFRONT_DISTRIBUTION!;
                ...
                if (getStorageInput.level === FileAccessLevel.Private && (!getStorageInput.id || getStorageInput.id.length === 0)) {
                    ...
                    // if there's an existent object, with valid metadata
                    if (metadata.Metadata && metadata.$metadata.httpStatusCode !== undefined && metadata.$metadata.httpStatusCode === 200) {
                        // retrieve the CloudFront distribution secrets for the logo file types, depending on the current region and environment
                        const cloudFrontLogoFilesPair = await secretsClient
                            .send(new GetSecretValueCommand(({SecretId: `${Constants.AWSPairConstants.LOGO_FILES_CLOUDFRONT_DISTRIBUTION_SECRET_NAME}-${process.env.ENV_NAME!}-${region}`})));

                        // check if the secrets for the CloudFront Distribution exist
                        if (cloudFrontLogoFilesPair.SecretString) {
                            /**
                             * If there is an expiration flag passed in, and it is true,
                             * then make the expiration to 1 hour, to coincide with the Cognito
                             * session expiration.
                            ...
                            return {
                                data: {
                                    url: getSignedUrl({
                                        url: `https://${logoCloudFrontDistributionDomain}/${objectKey}`,
                                        dateLessThan: expirationDate.toString(),
                                        keyPairId: process.env.MOONBEAM_LOGO_FILES_KEY_PAIR_ID!,
                                        privateKey: cloudFrontLogoFilesPair.SecretString!
                                    })
                                }
                            }
                        }
                        const cloudFrontDistributionPairMessage = `CloudFront Distribution secret string not available ${JSON.stringify(cloudFrontLogoFilesPair)}`;
                        console.log(`${cloudFrontDistributionPairMessage}`);
                        return {
                            errorMessage: cloudFrontDistributionPairMessage,
                            errorType: StorageErrorType.UnexpectedError,
